...
# ==============================================================================
# Layer 2: PROJ + GEOS + PostGIS, all from source on the same icx toolchain
# as Layer 1. Builds against the headers in /opt/pg18 from layer 1.
# Result image:  hartonomous/postgis:${POSTGIS_VERSION}
# ==============================================================================
...
# ---------- builder: needs full pg headers + dev tools + sources ----------
FROM ${IMG_NS}/postgres:${POSTGRES_VERSION} AS pgbase
# /opt/pg18 lives here. We'll copy it into the builder stage.
...
# Build PROJ/GEOS/PostGIS with stock gcc (Ubuntu 22.04 system toolchain).
# Same libstdc++ as the runtime image, no compiler-bug surface.
ENV CC=gcc CXX=g++ \
...
# initGEOS. GEOS installs without RPATH ("-- Set runtime path ... to \"\""),
# so without LD_LIBRARY_PATH the test executable fails to find libgeos.so at
# runtime and configure mistakenly concludes libgeos_c is missing.
...
# Bring PROJ + GEOS and PostGIS-installed bits.
