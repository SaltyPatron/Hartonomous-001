...
7. **For each tensor, dispatch by `(PrimitiveKind, ArchetypeTuple, TupleSlot)` to the appropriate primitive/tuple pass** (per [`docs/01-tensor-primitive-spec.md`](../../01-tensor-primitive-spec.md) §VI). Each pass emits typed attestation EDGES between existing content entities (typically two `word_form` tokens), with sign-aware `EdgeRatingEvent` per AP-31: `attestation_type ∈ {positive_evidence, negative_evidence, neutral_evidence}` per the 3-row P1d collapse (`sql/schema/seed/attestation_type.sql`); kind-of-evidence attribution lives on the `EdgeRatingEvent` fields `(PrimitiveCode, TupleCode, SlotCode, LayerIdx, HeadIdx, ExpertIdx, ModelSourceId, TensorHash, SourceTensorName)`, NOT as separate attestation_type rows. Working template: `src/Hartonomous.Decomposers/Safetensors/Passes/AttentionBlockTuplePass.cs`.
