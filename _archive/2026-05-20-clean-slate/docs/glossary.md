...
The geometric representation of an embedding-matrix row as a 4-dimensional point in concept space. Every ingested model's embeddings are projected into the same 4D frame via Laplacian eigenmap + Gram-Schmidt orthonormalization. Each row becomes a **firefly** stored in the `physicality` table with `physicality_type='embedding_firefly'` (where `embedding_firefly` is a *physicality_type code*, NOT an entity type — physicalities attach to existing `word_form` content entities per the 2026-05-08 architectural correction; see [`docs/00-substrate-spec.md`](00-substrate-spec.md) §VII). The species is the `word_form` entity; specimens are the per-model fireflies, distinguished by `entity_model_source`. Four dimensions is the minimum ambient dimension in which cross-model Voronoi consensus cells are guaranteed to have well-defined interiors for every shared token (see **Borsuk-Ulam**). Fireflies are a derived value-add side-channel for cross-model consensus visualization, NOT the inference mechanism (per AP-29; inference is A* over attestation edges). See [`docs/specs/engine/embedding-physicality.md`](specs/engine/embedding-physicality.md).
