...
**Substrate mechanism (corrected per [`docs/00-substrate-spec.md`](../00-substrate-spec.md) §VII):** Each ingested model with an embedding tensor contributes one POINTZM **firefly** per token to the substrate's 4D physicality jar (the `embedding_firefly` partition of `substrate.physicality`), **attached to the existing `word_form` content entity for that token** (the species). Llama-4's firefly for "cat," Qwen-3's firefly for "cat," GPT-4's firefly for "cat" — N POINTZMs in the 4D jar, all attached to the same `word_form` entity, distinguishable by `entity_model_source`. The 4D projection uses Laplacian eigenmap + Gram-Schmidt + L2 norm (Borsuk-Ulam d=4 minimum for non-trivial Voronoi cells). Cross-model comparison is Hausdorff over the firefly cluster attached to one species. There is NO separate `embedding_firefly` atom-class entity; the species is the `word_form` entity, the specimens are the per-model fireflies. Native-dim embedding rows (when needed for `EmbeddingLayerSynthesizer` during Substrate Synthesis synthesis) are derivable from substrate state via inverse Laplacian eigenmap or PCA over per-token attestation participation; the substrate's load-bearing storage is the per-token attestation edges (`model_concept_similarity` between word_form pairs with `attestation_type = model_embedding_proximity`), with fireflies as the side-channel for visualization and conventional embedding queries.

**Structural reason:** Conventional embeddings are model-specific. The substrate's content-addressed `word_form` entities (the same `cat` token is the same entity regardless of source model, via BLAKE3 of UTF-8 bytes through `SubstrateTextDecomposer`) make cross-model embeddings co-locate as fireflies on the same species. The 4D firefly is the substrate's interpretable cross-model representation, enabling conventional embedding queries (KNN, cosine) WITH cross-model consensus weighting that no vector DB can do. Embedding similarity becomes 4D geometric query (Fréchet, Hausdorff) over deterministic positions on a shared species. Fireflies are NOT the inference mechanism (per AP-29); inference is A* over attestation edges per [`.claude/rules/35-inference-and-godel.md`](../../.claude/rules/35-inference-and-godel.md).
