...
| `SafetensorsContainerDecomposer` (formerly `SafetensorsDecomposer`, scope-narrowed per [`docs/00-substrate-spec.md`](../00-substrate-spec.md) §V.1) | `D:\Models\hub\<model_dir>\` | `huggingface_model:<model_id>` | `ModelDecomp` | `tensor`, `model_architecture`, `tokenizer_model` entities (real model-side structural artifacts; **no phantom `attention_pattern` or `bpe_token` per spec §XII**); `in_model`, `in_layer`, `has_dtype`, `has_shape`, `has_token_in_tokenizer` edges; `tensor_role`, `model_architecture_class` junctions; firefly POINTZM physicality attached to existing `word_form` content entities (per spec §VII). Per-role attestation edges (`model_attention_pattern`, `model_concept_similarity`, `model_ffn_factor`) between `word_form` content entities are emitted by the dispatched layer-type decomposers (see [`docs/specs/decomposers/layer-type-library.md`](../specs/decomposers/layer-type-library.md)). |
