...
### AP-8: Pushing classification into substrate.entity for convenience reasons that bypass the attestation-edge surface

**Failure (pre-2026-05-14 framing, now superseded)**: Treating POS / sense / language / morph_feature / deprel as infrastructure (junction tables) where AI model attestations cannot fire on the same Glicko-2 surface as corpus attestations. The original AP-8 said "POS is NOT a node — it's a row in `substrate.pos`" — that's wrong as written; the corrected framing below.

**Current rule (2026-05-14 architectural correction)**: POS / sense / language / morph_feature / deprel ARE attestation kinds — same shape as AI model attestations on attention / FFN / embedding patterns. Every claim from every source competes on a unified Glicko-2 surface (`substrate.edge_significance`) via typed edges between content entities, with discrimination via (provenance × arena).

- Reference vocabulary rows (pos, deprel, sense, language, morph_feature, lexname, etc.) become **content-hashed substrate entities** reachable as edge targets.
- Per-entity classification evidence emits as **typed substrate edges**: `has_pos(word_form, pos_entity)`, `has_sense(lemma, synset)` (already exists), `has_language(entity, language_name)`, `has_morph_feature(word_form, morph_feature_entity)`, `has_deprel_pattern(word_form, deprel_entity)`.
- The Glicko-2 truth about classifications lives in `substrate.edge_significance` per arena — NOT in `entity_pos.mu` / `pattern_deprel.mu` junctions. (provenance × arena) discriminates UD's POS observation from Wiktionary's POS attestation from Llama's lm_head row's noun-context strength.
- Old junction tables (`entity_pos`, `pattern_deprel`, `entity_language`, `entity_morph_feature`, `entity_lexname`) remain as **analytics caches** (denormalized for index-locality on frequent classification-lookup queries per `docs/00-substrate-spec.md` §X.1), but the substrate's authoritative classification consensus lives on the edge surface, not in the junctions.

**Forbidden shape**: pushing classification rows into `substrate.entity` *for ergonomic convenience that bypasses the attestation-edge surface*. Classifications-as-entities-targets-of-attestation-edges is the canonical path; classifications-stuffed-into-entity-rows-without-typed-edges is still wrong because it loses the per-arena Glicko-2 competition surface that makes consensus emerge.

**Why**: AI model attestations on classification claims (Llama's lm_head row attesting `rake` is noun-like) need to compete with corpus attestations (UD attesting `rake → NOUN` from 14,000 dependency parses) on the SAME Glicko-2 ladder. Splitting into separate junction Glicko ladders breaks cross-source consensus. The arena's leaderboard at query time IS the substrate's quantitative answer to "what POS is this word in this domain."

**Citation**: `docs/specs/sql/infrastructure-vs-substrate.md`; `docs/00-substrate-spec.md` §IV (Glicko-2 surfaces — collapsing the four-surface framing into the unified edge_significance + entity_significance surface is P1g in the build plan).

### AP-37: End-of-phase backfill (the phase-boundary post-pass anti-pattern)

**Failure**: Edge geometry left NULL at INSERT to be backfilled at phase end by `PopulateEdgeTrajectoriesAsync`. Per-arena Glicko-2 priming run as `PrimeAllSignificanceAsync` at phase end. Anything described as a "phase post-pass" or "end-of-phase backfill" or "phase boundary commit." Lossy windows where `substrate.edge.geom IS NULL` rows are queryable but invisible to Fréchet / Hausdorff / GiST-bbox queries; where new arenas can't compete on existing edges until the next phase end; where the substrate is incoherent for the duration of the window.

**Rule**: Drain completion (not phase completion) is the post-pass trigger. The substrate is continuously queryable — phases are an orchestration convenience for the runner, NOT a substrate boundary. Every `IIngestionPipeline.DrainPendingAsync` invocation atomically: waits for all channels quiescent, fires `substrate.populate_edge_trajectories` (bulk JOIN against composite-btree-indexed `substrate.entity_by_hash_prefix` for cross-batch participants), then fires `substrate.prime_unprimed_edges_chunk` cross-producting against every current arena in `substrate.significance_context`. After `DrainPendingAsync` returns, no edge sits with NULL geom and no arena has unprimed significance rows.

**End-state (P1f-followup target)**: edge geom built inline at edge INSERT via single combined `INSERT ... SELECT ... ST_MakeLine(array_agg(ST_MakePoint(bb_pack_*, ...) ORDER BY role_position))` SQL — no NULL-geom window at all, even between INSERT and the immediately-following bulk geom build. Per-arena Glicko-2 priming via `INSERT ... SELECT` CROSS JOIN `substrate.significance_context` in the same transaction. New arena added at runtime → substrate trigger/function back-primes existing edges via a single bulk `INSERT ... SELECT`.

**Why**: the substrate is supposed to be the universal absorbent + continuously queryable + no context window — a user prompt ingested at runtime can't wait for a "phase end" before becoming traversable. Inference firing between INSERT and backfill walks broken substrate state and produces garbage. The "BackgroundSignificancePrimer" / staging-schema pattern was removed (see `0ce4e5e`) for the same reason — substrate state must be atomic-on-commit, not "eventually consistent."

**Citation**: `.claude/rules/20-sql-and-ingestion.md` (drain-completion as the post-pass trigger); `docs/00-substrate-spec.md` §XII (no phase concept as a substrate boundary).

### AP-38: Modality-specific attestation_type pidgeonholing

**Failure**: `attestation_type` reference table carries 27+ rows discriminating evidence by modality / model mechanism / source kind (`model_attention_qk_pattern`, `model_ffn_full_path`, `model_lm_head_projection`, `model_cross_modal_alignment`, `corpus_co_occurrence_window`, `lexical_curated_relation`, `ud_dependency_observation`, etc.). Every new modality / new AI architecture / new source kind requires extending the enum. The substrate is a finite enumeration where it claims to be universal.

**Rule**: The (provenance × arena) tuple discriminates evidence by SOURCE (provenance_id) and DOMAIN (context_type_id). The `attestation_type` column on `substrate.edge_significance` + `substrate.entity_significance` carries ONLY the sign-bearing discriminator (positive_evidence / negative_evidence / neutral_evidence) — the same information Glicko-2 score (1.0 / 0.0 / 0.5) already encodes. attestation_type is on the deprecation path; the column itself drops in P1e-followup once the IIngestionBatch.AddSignificance signature stops threading it and all decomposer call-sites have migrated.

**Why**: the substrate's universal property requires that every claim about content from every source be the same shape of evidence on the same Glicko-2 surface, with discrimination via provenance + arena. A new AI model ingested next month doesn't extend the attestation_type enum — it gets a new provenance row. A new domain of evaluation doesn't extend the enum — it gets a new significance_context row. The substrate accumulates indefinitely without schema changes.

**Citation**: `sql/schema/seed/attestation_type.sql` (P1d collapse 2026-05-14); `sql/schema/functions/resolve_attestation_type_id.sql` (graceful-fallback path for pre-collapse decomposer calls).
...
**Rule**: `BaseDecomposer.ComputeHash` accepts content bytes only. `ComputeMerkleHash` accepts ordered child hashes. `ComputeEdgeHash` accepts `(edge_type_id, participant_hashes)`. Placement lives in the composition `LINESTRINGZM` physicality vertex stream — the Y mantissa carries `(ordinal, rle_count)` via `bb_pack_ordinal_rle` — on typed edges (`has_source`, `in_model`, edge_member.role_position), on model-source tables, or on provenance. There is no `substrate.sequence` table; the geometry IS the indexed child manifest. Same content in two places = one entity referenced from two trajectories.
...
**Rule**: Per-role units of Track 2 transformation tensors **manifest as typed attestation EDGES between existing content entities** (typically two `word_form` tokens, or a token and a `visual_concept` for cross-modal models). The `edge_type_id` encodes the relationship; provenance + arena + rating attribution encode source/mechanism/domain; `attestation_type` is only the sign-bearing discriminator while that column is on the deprecation path. The edge's `LINESTRINGZM` trajectory is the unit's spectral fingerprint; the edge's per-arena Glicko mu carries the strength. Cross-model corroboration: same `(edge_type_id, role-ordered participant hashes)` -> same edge hash -> multiple models fire separate rating events on the same edge (sigma tightens; no duplicate edges). The phantom entity types listed above are absent from `sql/schema/seed/entity_type.sql`; current seed count is 34 including reference-vocabulary and UCD-property entity targets. The phantom decomposer passes (`FfnNeuronPass`, `EmbeddingPositionPass`, etc.) have also been replaced by the layer-type tuple/primitive passes. Working template: `src/Hartonomous.Decomposers/Safetensors/Passes/TokenAttentionEdgePass.cs`.

**Why**: Phantom per-role-unit entities defeat content-addressed identity — they're hashed by per-tensor-row content, so two models almost never collapse. The substrate's truth fragments into per-source debris instead of accumulating cross-model consensus. Substrate Synthesis synthesis recomposition becomes impossible because there's no consensus surface to synthesize from. Crystal ball / mechanistic interpretability queries can't compare across models because each model's units are siloed.

**Citation**: [`docs/00-substrate-spec.md`](../../docs/00-substrate-spec.md) §III, §XII; `sql/schema/seed/entity_type.sql` (34 current rows verified 2026-05-19; phantom rows absent); `sql/schema/seed/attestation_type.sql` (3 sign-bearing rows); `src/Hartonomous.Decomposers/Safetensors/Passes/TokenAttentionEdgePass.cs`.
...
**Citation**: [`docs/specs/engine/godel-engine.md`](../../docs/specs/engine/godel-engine.md) (full spec — orchestrator framing; OODA at three scales; reasoning strategies; operational boundaries); [`.claude/rules/35-inference-and-godel.md`](35-inference-and-godel.md); [`docs/substrate-bond.md`](../../docs/substrate-bond.md) Properties 1 and 2.
