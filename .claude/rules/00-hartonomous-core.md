...
**Entity table**: `substrate.entity(hash substrate.hash_value PRIMARY KEY)` — one column, no `id`, no `entity_type_id`. Classifications go in `substrate.entity_classification(entity_hash, entity_type_id, provenance_id)`. Reference vocabulary (`pos`, `language`, `entity_type`, `edge_type`, etc.) is separate infrastructure — not substrate content.
...
**No phase boundaries / no backfill** (AP-37): Drain completion triggers post-passes (`PopulateEdgeTrajectoriesAsync` + `PrimeAllSignificanceAsync` invoked inside `StreamingIngestionPipeline.DrainPendingAsync`), not phase ends. Decomposers run independently; the pipeline ingests / corroborates / primes continuously. P1f-followup target: fully inline at INSERT-SELECT with no NULL-geom window even briefly.
