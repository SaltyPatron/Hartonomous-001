...
///
/// (parent_type, parent_hash, ordinal) is unique by construction; the drain
/// function ON CONFLICT DO NOTHING is for re-ingestion idempotence.
/// </summary>
public sealed record SequenceRecord(
    string ParentEntityTypeCode,
    byte[] ParentEntityHash,
    int Ordinal,
    string ChildEntityTypeCode,
