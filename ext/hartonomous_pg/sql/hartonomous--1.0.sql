...
-- (23) SETOF inventory accessors removed 2026-05-17 (Gate 1 Task #22).
-- substrate.ucd_general_categories / ucd_scripts / ucd_blocks /
-- ucd_break_properties were SQL bindings for populate_*_from_ext, which
-- are gone. Reference-vocabulary population now happens client-side via
-- BlobUcdPropertyAccessor + the canonical seed inserts under
-- sql/schema/seed/. Per Principle 1 — blob and substrate are siblings.
...
-- Bulk SRFs (ucd_codepoints / ucd_codepoints_in_block / _in_script /
-- _with_gc) removed 2026-05-17 (Gate 1 Task #22) along with the
-- multi-codepoint relation SRFs (ucd_named_sequences,
-- ucd_emoji_sequences, ucd_emoji_zwj_sequences,
-- ucd_standardized_variants, ucd_confusables, ucd_idna_mapping,
-- ucd_cjk_radicals). These bound the populate_*_from_ext server-side
-- ingestion path that is now retired. UnicodeDecomposer reads the UCD
-- source directly via BlobUcdPropertyAccessor + IIngestionPipeline.
-- substrate.cp_atom() remains for ad-hoc single-cp lookup.
...
-- ── sql/schema/tables/core/entity_significance_p0.sql ───────────────────────────────────────
CREATE TABLE substrate.entity_significance_p0
    PARTITION OF substrate.entity_significance
    FOR VALUES WITH (modulus 8, remainder 0);

-- ── sql/schema/tables/core/entity_significance_p1.sql ───────────────────────────────────────
CREATE TABLE substrate.entity_significance_p1
    PARTITION OF substrate.entity_significance
    FOR VALUES WITH (modulus 8, remainder 1);

-- ── sql/schema/tables/core/entity_significance_p2.sql ───────────────────────────────────────
CREATE TABLE substrate.entity_significance_p2
    PARTITION OF substrate.entity_significance
    FOR VALUES WITH (modulus 8, remainder 2);

-- ── sql/schema/tables/core/entity_significance_p3.sql ───────────────────────────────────────
CREATE TABLE substrate.entity_significance_p3
    PARTITION OF substrate.entity_significance
    FOR VALUES WITH (modulus 8, remainder 3);

-- ── sql/schema/tables/core/entity_significance_p4.sql ───────────────────────────────────────
CREATE TABLE substrate.entity_significance_p4
    PARTITION OF substrate.entity_significance
    FOR VALUES WITH (modulus 8, remainder 4);

-- ── sql/schema/tables/core/entity_significance_p5.sql ───────────────────────────────────────
CREATE TABLE substrate.entity_significance_p5
    PARTITION OF substrate.entity_significance
    FOR VALUES WITH (modulus 8, remainder 5);

-- ── sql/schema/tables/core/entity_significance_p6.sql ───────────────────────────────────────
CREATE TABLE substrate.entity_significance_p6
    PARTITION OF substrate.entity_significance
    FOR VALUES WITH (modulus 8, remainder 6);

-- ── sql/schema/tables/core/entity_significance_p7.sql ───────────────────────────────────────
...
-- ── sql/schema/tables/core/edge_significance_p0.sql ───────────────────────────────────────
CREATE TABLE substrate.edge_significance_p0
    PARTITION OF substrate.edge_significance
    FOR VALUES WITH (modulus 8, remainder 0);

-- ── sql/schema/tables/core/edge_significance_p1.sql ───────────────────────────────────────
CREATE TABLE substrate.edge_significance_p1
    PARTITION OF substrate.edge_significance
    FOR VALUES WITH (modulus 8, remainder 1);

-- ── sql/schema/tables/core/edge_significance_p2.sql ───────────────────────────────────────
CREATE TABLE substrate.edge_significance_p2
    PARTITION OF substrate.edge_significance
    FOR VALUES WITH (modulus 8, remainder 2);

-- ── sql/schema/tables/core/edge_significance_p3.sql ───────────────────────────────────────
CREATE TABLE substrate.edge_significance_p3
    PARTITION OF substrate.edge_significance
    FOR VALUES WITH (modulus 8, remainder 3);

-- ── sql/schema/tables/core/edge_significance_p4.sql ───────────────────────────────────────
CREATE TABLE substrate.edge_significance_p4
    PARTITION OF substrate.edge_significance
    FOR VALUES WITH (modulus 8, remainder 4);

-- ── sql/schema/tables/core/edge_significance_p5.sql ───────────────────────────────────────
CREATE TABLE substrate.edge_significance_p5
    PARTITION OF substrate.edge_significance
    FOR VALUES WITH (modulus 8, remainder 5);

-- ── sql/schema/tables/core/edge_significance_p6.sql ───────────────────────────────────────
CREATE TABLE substrate.edge_significance_p6
    PARTITION OF substrate.edge_significance
    FOR VALUES WITH (modulus 8, remainder 6);

-- ── sql/schema/tables/core/edge_significance_p7.sql ───────────────────────────────────────
CREATE TABLE substrate.edge_significance_p7
    PARTITION OF substrate.edge_significance
    FOR VALUES WITH (modulus 8, remainder 7);

-- ── sql/schema/tables/core/recipe.sql ───────────────────────────────────────
...
-- ── sql/schema/tables/core/recipe_name.sql ───────────────────────────────────────
...
-- ── sql/schema/tables/reference/tokenizer_family.sql ───────────────────────────────────────
...
-- ── sql/schema/tables/reference/tokenizer_marker_role.sql ───────────────────────────────────────
...
-- ── sql/schema/tables/junctions/tokenizer_family_marker.sql ───────────────────────────────────────
...
-- ── sql/schema/tables/reference/tokenizer_special_token_kind.sql ───────────────────────────────────────
...
-- ── sql/schema/tables/core/tokenizer_special_token.sql ───────────────────────────────────────
...
-- ── sql/schema/indexes/tokenizer_special_token_assigned_vocab_idx.sql ───────────────────────────────────────
...
-- ── sql/schema/indexes/tokenizer_special_token_by_kind_idx.sql ───────────────────────────────────────
...
-- ── sql/schema/functions/write_entities.sql ───────────────────────────────────────
...
-- ── sql/schema/functions/write_entity_classifications.sql ───────────────────────────────────────
...
-- ── sql/schema/functions/write_physicalities.sql ───────────────────────────────────────
...
-- ── sql/schema/functions/write_edges.sql ───────────────────────────────────────
...
-- ── sql/schema/functions/write_edge_members.sql ───────────────────────────────────────
...
-- ── sql/schema/functions/merkle_tree_filter.sql ───────────────────────────────────────
...
-- ── sql/schema/functions/save_recipe.sql ───────────────────────────────────────
...
-- ── sql/schema/functions/get_recipe_by_name.sql ───────────────────────────────────────
...
-- ── sql/schema/functions/get_recipe_by_hash.sql ───────────────────────────────────────
...
-- ── sql/schema/functions/list_recipes.sql ───────────────────────────────────────
...
-- ── sql/schema/seed/recipe_starter.sql ───────────────────────────────────────
...
-- ── sql/schema/seed/tokenizer_family.sql ───────────────────────────────────────
...
-- ── sql/schema/seed/tokenizer_marker_role.sql ───────────────────────────────────────
INSERT INTO substrate.tokenizer_marker_role (id, code) VALUES
    (1, 'leading_space'),
    (2, 'continuation_prefix'),
    (3, 'continuation_suffix'),
    (4, 'byte_encoded_space'),
    (5, 'byte_encoded_newline'),
    (6, 'metaspace');

-- ── sql/schema/seed/tokenizer_family_marker.sql ───────────────────────────────────────
...
-- ── sql/schema/seed/tokenizer_special_token_kind.sql ───────────────────────────────────────
...
-- ── sql/schema/seed/tokenizer_special_token.sql ───────────────────────────────────────
