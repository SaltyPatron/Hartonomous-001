...
while (($#)); do
    case "$1" in
        ...
        -h|--help)
            cat <<'USAGE'
Usage: scripts/linux/phases.sh list|status|run [--phase PHASE] [--source PATH] [--skip-deps] [--force] [--dry-run] [--no-build]
...
case "$action" in
    ...
    run)
        ...
        run_cli "${cli_prefix[@]}" phases run --phase "$phase" --connection "$HARTONOMOUS_DB" --source "$source_root" "${extra[@]}"
        ;;
    *)
