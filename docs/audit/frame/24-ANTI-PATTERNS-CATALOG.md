...
| AP-5 | Round-trip recomposer as Substrate Synthesis | Framing recomposer as "ingest a model, export the same model"; distillation = WHERE clause export; substrate never stored the gradient noise so exported model is denser than source |
...
| AP-28 | Round-trip recomposer as Substrate Synthesis (code shape of AP-5) | `SafetensorsRecomposer.AssembleTensorBytesAsync` walks `has_constituent` phantom per-role children + scatters; single-source phantom-scatter cannot benefit from cross-model consensus; must replace with synthesis-from-consensus per `frame/09-RECOMPOSERS-SYNTHESIS.md` |
