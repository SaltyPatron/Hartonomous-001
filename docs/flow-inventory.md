...
| **Pipeline mode** | Batch seed ingestion → `IIngestionPipeline` |
| **Steps** (corrected per [`docs/00-substrate-spec.md`](00-substrate-spec.md) §V) | 1. Scan model directory → 2. Per model: read `config.json` → detect architecture (via `ModelConfigDecomposer`) → 3. Read `.safetensors` headers (JSON metadata, no GPU) → 4. Classify tensors by role via `TensorClassifier` (embedding, attention_q/k/v/o, ffn_up/gate/down, norm, head, etc.) → 5. Create entities: `model_architecture`, `tensor` (real structural artifact entities; **no phantom `attention_pattern` or `bpe_token`** — those are deprecated per spec §XII) → 6. Create edges: `in_model`, `in_layer`, `has_dtype`, `has_shape` → 7. Populate `model_architecture_class` junction → 8. Populate `tensor_tensor_role` junction → 9. Tokenizer decomposition via `HuggingFaceTokenizerDecomposer`: route every vocab token's bytes through `SubstrateTextDecomposer.EmitStatic` → produces `word_form` content entities (collapsing across all models that share the token) → 10. Run analysis passes (per-tensor analysis surfaces): SVD spectrum, eigenvalue, sparsity, weight distribution, layer similarity → 11. **Dispatch each tensor to its layer-type decomposer** (per [`specs/decomposers/layer-type-library.md`](specs/decomposers/layer-type-library.md)): universal (`AttentionQkvLayerDecomposer`, `AttentionVoLayerDecomposer`, `FfnLayerDecomposer`, `EmbeddingLayerDecomposer`, `LmHeadLayerDecomposer`, `LayerNormLayerDecomposer`, `MoeRouterLayerDecomposer`, `MoeExpertLayerDecomposer`, `LoRAAdapterLayerDecomposer`) or specialist (`CrossAttentionLayerDecomposer`, `ConvLayerDecomposer`, `ViTPatchAttentionLayerDecomposer`, `CodecRvqLayerDecomposer`, `DetectionHeadLayerDecomposer`, `DiffusionUnetLayerDecomposer`). Each decomposer emits typed attestation EDGES between existing content entities (typically two `word_form` tokens) with `attestation_type` (per `sql/schema/seed/attestation_type.sql`) on the rating event distinguishing the kind of model evidence. → 12. Side-effect: `EmbeddingLayerDecomposer` emits one POINTZM firefly per token to the `substrate.physicality` jar attached to the existing `word_form` entity (per spec §VII; firefly value-add for cross-model consensus visualization, NOT inference). → 13. Tensor-level physicality from sparsity profile / weight distribution. |
| **Entities** | `model_architecture`, `tensor`, `tokenizer_model` (real model-side structural artifacts per spec §II.1). Token-side content entities (`word_form` for each vocab token) are produced by `HuggingFaceTokenizerDecomposer` via `SubstrateTextDecomposer`; they are content-addressed and shared across all models that have the token. **No phantom per-role-unit entities.** |
| **Edges** | `in_model`, `in_layer`, `has_dtype`, `has_shape`, `has_token_in_tokenizer`, plus per-role attestation edges (`model_attention_pattern`, `model_concept_similarity`, `model_ffn_factor`) between `word_form` content entities, with `attestation_type` per-rating-event metadata. |
