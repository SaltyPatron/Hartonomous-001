...
| `attention_pattern_confidence` | `model_attention_pattern` edges between `word_form` content entities (per `sql/schema/seed/edge_type.sql:84-90`) with `attestation_type ∈ {model_attention_qk_pattern, model_attention_vo_pattern}` per `sql/schema/seed/attestation_type.sql`; layer/head metadata on the rating event | How confident we are that the attention head's QK pattern across these tokens reflects a real relation. Cross-model corroboration from many models attesting the same token pair tightens sigma. The `Build-a-bear` `AttentionQkvLayerSynthesizer` queries this arena's mu when projecting consensus into target attention tensors (per [`docs/specs/recomposers/synthesis-library.md`](../recomposers/synthesis-library.md)). |
