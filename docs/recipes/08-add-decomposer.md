...
- **DON'T** emit phantom per-role-unit entities** (`ffn_neuron`, `attention_head`, `attention_pattern`, `embedding_position`, `logit_projection`, `moe_route`, `moe_expert_neuron`, `attention_archetype`, `svd_rank_component`, `codec_codevector`, `audio_codec_filter`, `bbox_projection`, `class_projection`, `conformer_component`, `conv_filter`, `diffusion_component`, `lora_component`, `modality_basis_vector`, `moe_route_direction`, `object_query_slot`, `vision_feature_direction`, `residual_direction`, `archetype`). These entity types are deprecated by the 2026-05-08 architectural correction. Per-role units of Track 2 transformation tensors **manifest as typed attestation EDGES between existing content entities** (typically two `word_form` tokens). See AP-25 in `.claude/rules/45-anti-patterns.md`, [`docs/00-substrate-spec.md`](../00-substrate-spec.md) §III, and the canonical layer-type decomposer library at [`docs/specs/decomposers/layer-type-library.md`](../specs/decomposers/layer-type-library.md). Working template: `src/Hartonomous.Decomposers/Safetensors/Passes/TokenAttentionEdgePass.cs`.
- **DON'T** organize decomposers by downstream modality (`TextModelDecomposer`, `VisionModelDecomposer`, etc.). Decomposers organize by tensor layer-type (universal `AttentionQkvLayerDecomposer` works for any architecture with attention, etc.). Modality is a downstream USE property; layer-type is what the tensor math IS. See AP-26.
- **DON'T** skip bulk-existence-check before emit. Use `IIngestionPipeline.GetExisting{EntityHashes,EntityClassifications,Edges,Physicalities,SequenceRows}Async` ONCE per kind per chunk before emitting; emit only the diff. See AP-19.
- **DON'T** treat row-identity dedup and rating-event dedup as the same — `attestation_type_id` stratifies rating rows so cross-source corroboration accumulates separately on the same edge. See AP-22.
