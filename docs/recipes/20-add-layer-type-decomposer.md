...
2. **The attestation type exists in `sql/schema/seed/attestation_type.sql`.** If your decomposer emits a new kind of model evidence, add the attestation_type seed row first. Use the existing 18 model_* attestation types (`model_attention_qk_pattern`, `model_ffn_full_path`, etc.) if your evidence kind matches one of them.
